<x-app-layout>
    ...
    <form action="{{route('categories.update', $category)}}" method="POST" class="p-5 shadow-md  bg-white rounded-xl mt-4">
        @csrf
        @method('PATCH')
        <div class="grid grid-cols-2 gap-6 gap-y-8 h-full">
            <div>
                ...
                <x-input id="name" class="block w-full" type="text" name="name" :value="old('name', $category->name)" required autofocus />
