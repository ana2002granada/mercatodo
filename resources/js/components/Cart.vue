<template>
    <div>
      ...
      <button @click="isOpen = !isOpen" class="bg-gray-200 hover:bg-gray-300 rounded-full p-2 flex items-center text-gray-600 focus:outline-none">
            <em class="fas fa-cart-plus"></em>
        </button>
      <span class="flex absolute h-3 w-3 top-0 right-0 -mt-1 -mr-1" v-if="items.length">
    ...
    <div :class="isOpen ? 'translate-x-0 ease-out' : 'translate-x-full ease-in'" class="pb-44 overflow-auto z-50 fixed right-0 top-0 max-w-xs w-full px-6 py-4 transition duration-300 transform mt-28 overflow-y-auto bg-white border-l-2 border-gray-300 flex flex-col flex-grow h-full fixed ">
...
export default {
    ...
    mounted() {
        console.log(localStorage.shoppingCar);
        ...
        this.$root.$on('add-product', (data) => {
            ...
            if (product) {
                console.log('Entré al if del mounted');
                product.count = product.count + 1;
                product.total = product.total + product.price;
                localStorage.setItem( 'shoppingCar' , JSON.stringify(this.items));

            } else {
                console.log('Entré al else del mounted');
                this.items.push({
                    'id': data.id,
                    'name': data.name,
                    'count': 1,
